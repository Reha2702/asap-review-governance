...
def aspect_summary(frame: pd.DataFrame) -> pd.DataFrame:
    ...
    for column in detect_aspect_columns(frame):
        ...
        rows.append(
            {
                "方面": ASPECT_NAMES[column],
                "提及量": len(mentioned),
                "负面率": (mentioned == -1).mean(),
                "正面率": (mentioned == 1).mean(),
            }
        )
...
data = load_data()
if data.empty:
    st.error("尚未生成数据。请依次运行 `python download_data.py` 和 `python prepare_data.py`。")
    st.stop()
...
st.markdown(
    ...
    <header class="archive-topbar">
      <div class="archive-brand">
        <span class="archive-brand-mark">评</span>
        <span>评鉴志</span>
        <small>中文评论治理档案库</small>
      </div>
      <div class="dataset-status">
        <span class="status-dot"></span>
        ASAP 数据集已载入 <strong>{len(data):,}</strong> 条评论
      </div>
    </header>
...
with st.sidebar:
    ...
    selected_risk = st.multiselect(
        "风险等级", data["risk_level"].dropna().unique(), default=data["risk_level"].dropna().unique()
    )
    selected_quality = st.multiselect(
        "内容质量", data["quality_level"].dropna().unique(), default=data["quality_level"].dropna().unique()
    )
    keyword = st.text_input("评论关键词")

filtered = data[
    data["risk_level"].isin(selected_risk) & data["quality_level"].isin(selected_quality)
].copy()
...
st.markdown(
    '<div class="section-index"><span>01</span><strong>治理分析</strong></div>',
    unsafe_allow_html=True,
)

feedback_tab, quality_tab, risk_tab, strategy_tab, explorer_tab = st.tabs(
    ["用户反馈", "内容质量", "风险识别", "审核策略", "评论明细"]
)
...
with feedback_tab:
    ...
    right.plotly_chart(
        px.bar(
            summary.sort_values("负面率", ascending=False).head(12),
            x="负面率",
            y="方面",
            orientation="h",
            title="负面反馈集中维度",
            color_discrete_sequence=[COLORS["red"]],
        ).update_layout(yaxis={"categoryorder": "total ascending"}, xaxis_tickformat=".0%"),
        use_container_width=True,
    )
...
with quality_tab:
    ...
    left.plotly_chart(
        px.histogram(
            ...
            nbins=20,
            title="评论质量得分分布",
            color_discrete_map={"优质": COLORS["green"], "合格": COLORS["yellow"], "低质": COLORS["red"]},
        ),
        use_container_width=True,
    )
    ...
    st.dataframe(
        filtered.nsmallest(30, "quality_score")[["review_text", "quality_score", "quality_level", "quality_reasons"]],
        use_container_width=True,
        hide_index=True,
    )
...
with risk_tab:
    ...
    st.plotly_chart(
        px.bar(counts, x="风险类型", y="命中量", title="规则命中分布", color_discrete_sequence=[COLORS["red"]]),
        use_container_width=True,
    )
    st.dataframe(
        risk_data[["review_text", "risk_level", "risk_types", "risk_reason", "audit_action"]].head(100),
        use_container_width=True,
        hide_index=True,
    )
...
with strategy_tab:
    ...
    st.plotly_chart(
        px.bar(action_counts, x="审核动作", y="评论量", color="审核动作", title="策略模拟结果"),
        use_container_width=True,
    )
...
with explorer_tab:
    search_columns = [
        "review_text",
        *([star_column] if star_column else []),
        "quality_score",
        "quality_level",
        "risk_level",
        "risk_types",
...
st.divider()
st.markdown(
    '<div class="section-index"><span>02</span><strong>单条评论即时诊断</strong></div>',
    unsafe_allow_html=True,
)
...
if st.button("分析这条评论", type="primary"):
    ...
    st.json(
        {
            "质量得分": int(result["quality_score"]),
            "质量等级": result["quality_level"],
            "质量依据": result["quality_reasons"],
            "风险等级": result["risk_level"],
            "风险类型": result["risk_types"],
            "审核动作": result["audit_action"],
        },
        expanded=True,
    )
